...
clc;
baileyModelSBMLfileName = 'testbibi33rxn.xml';
Krambeckmodel = glycanNetSBMLread(baileyModelSBMLfileName);
...
glycanNetViewer(Krambeckmodel,options,0);
...
glycanNetViewer(Krambeckmodel,options); 
...
Krambeckmodel.glycanNet_sbmlmodel = convertToSingleCompt(Krambeckmodel.glycanNet_sbmlmodel);
% ubODEFileName = regexprep(UB1997Model.glycanNet_sbmlmodel.name,'.m','');  
ubODEFileName = 'krambeckv1';
if(exist(ubODEFileName,'file')~=2)
   WriteODEFunction(Krambeckmodel.glycanNet_sbmlmodel,ubODEFileName);
end
...
rehash toolboxcache;
[tspan, speciesConc, speciesnames] = glycanDSim(Krambeckmodel,ubODEFileName,endTime, tsteps,options,1);

binames={'M3gn2_trans_golgi_network';'M3gn2g_trans_golgi_network';...
    'M3gn2gnb_trans_golgi_network';'M3gn2gnbg_trans_golgi_network'}; %,'M3GnGnb_TGN','M3GnGnbG_TGN'};
biindex   =  findbynameByBaileyModel(speciesnames,binames);
    
triprimenames={'M3gn3b_trans_golgi_network';'M3gn3bg_trans_golgi_network';...
    'M3gn3bgnb_trans_golgi_network';'M3gn3bgnbg_trans_golgi_network'}; 
...
for i=1:length(qpArray);
     ...
     options = odeset('RelTol',1e-12,'AbsTol',1e-6);
     [tspan, speciesConc, speciesnames] = glycanDSim(Krambeckmodel,ubODEFileName,endTime, tsteps,options);
     [sumbiconc,sumtriconc]             = getBiTriConcFromBaileyModel(speciesConc,biindex, triindex);
     plotBi(i)                          = sumbiconc;
