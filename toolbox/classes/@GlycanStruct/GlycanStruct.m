classdef GlycanStruct < handle
    ...
    methods
        ...
        function highmannose = ishighmannose(obj)
            ithspeciescomp = obj.getComposition;
            if(isfield(ithspeciescomp,'freeEnd'))
                ithspeciescomp=rmfield(ithspeciescomp,'freeEnd');
            end
            highmancond1   = isfield(ithspeciescomp,'GlcNAc') && (ithspeciescomp.GlcNAc==2);
            highmancond2   = isfield(ithspeciescomp,'Man') && (ithspeciescomp.Man>3);
            highmancond3   = length(fieldnames(ithspeciescomp))==2;
         
            if(highmancond1&&highmancond2&&highmancond3)
                highmannose = 1;
            else
                highmannose = 0;
            end
        ...
        function hybrid = ishybrid(obj)
            ithspeciescomp = obj.getComposition;
            if(isfield(ithspeciescomp,'freeEnd'))
                ithspeciescomp=rmfield(ithspeciescomp,'freeEnd');
            end
           hybridstruct1 = isfield(ithspeciescomp,'GlcNAc')&&(ithspeciescomp.GlcNAc>=3);
           hybridstruct2 = isfield(ithspeciescomp,'Man')&&(ithspeciescomp.Man>3);
           hybridstruct3 = length(fieldnames(obj))>2;
           if(hybridstruct1&&hybridstruct2&&hybridstruct3)
               hybrid = 1;
           else
               hybrid = 0;
           end
        ...
        function complextype = iscomplex(obj)
            ithspeciescomp = obj.getComposition;
            if(isfield(ithspeciescomp,'freeEnd'))
                ithspeciescomp=rmfield(ithspeciescomp,'freeEnd');
            end
           complexcond1  = isfield(ithspeciescomp,'GlcNAc')&&(ithspeciescomp.GlcNAc>=3);
           complexcond2  = isfield(ithspeciescomp,'Man')   &&(ithspeciescomp.Man==3);

           complexprecursor1  = length(fieldnames(ithspeciescomp))==2 &&...
                                isfield(ithspeciescomp,'GlcNAc')&&...
                                isfield(ithspeciescomp,'Man');

           complexprecursor2  = length(fieldnames(obj))==3 &&...
                                isfield(ithspeciescomp,'GlcNAc')&&...
                                isfield(ithspeciescomp,'Man') &&...
                                isfield(ithspeciescomp,'Fuc');            

           if ~(complexcond1 && complexcond2)
               complextype.complex = 0;
               complextype.branchnum = 0;
           else
               if(complexprecursor1 ||complexprecursor2 )
                   complextype.complex = -1;
               else
                   complextype.complex = 1;
               end
               
               nredEndRes = obj.getNonRedEndResidue;
               numBranch = 0;
               for i = 1 : length(nredEndRes)
                    if(strcmpi(nredEndRes{1,i}.residueType.name,'GlcNAc'))
                        numBranch = numBranch + 1;
                    elseif(strcmpi(nredEndRes{1,i}.residueType.name,'NeuAc'))
                        numBranch = numBranch + 1;
                    elseif(strcmpi(nredEndRes{1,i}.residueType.name,'Gal'))
                        numBranch = numBranch + 1;                    
                    end
               end
               complextype.branchnum = numBranch;
           end
        end        
    end
    ...
    methods
        function addBracket(obj)
            if(isempty(obj.bracket))
                obj.bracket = GlycanResidue.createBracket;            
            end
        end
        
        function addResiduesToBracket(obj,residues)
            for i = 1 : length(residues)
                addResidueToBracket(obj,residues(i,1)); % use unknown bonds
            end
        end
        
        function addResidueToBracket(obj,residueToAttach,varargin)
             ...
             if(isempty(obj.bracket))
                obj.addBracket;
                addResidue(obj,obj.bracket,residueToAttach,anomerchar,bonds);
             end
             
             if(isa(residueToAttach,'GlycanResidue'))
                addResidue(obj,obj.bracket,residueToAttach,bonds);
             elseif(isa(residueToAttach,'ResidueType'))
                addResidue(obj,obj.bracket,residueToAttach,anomerchar,bonds);
             end    
        end
        
        function removeBracket(obj)
            obj.bracket = [];
        end        
        
%        function residueToRootChar = getentiretargetbranch(obj,residue)
%             residueToRootChar   = obj.getresiduetoroot(residue.getParent);
%             parentresidue = residue.getParent;
%             Children      = parentresidue.getChildren;
%             ischild       = ~isempty(parentresidue.getChildren);
%             
%             while(ischild)
%                 for l = 1 : length(Children)
%                     ithChildren = Children(l);
%                     if (isequal(ithChildren.residueType.name,residue.residueType.name))
%                         continue
%                     else
%                         residueTypeChar =  [ithChildren.stereoConfig.symbol,'-'];
%                         residueTypeChar = [residueTypeChar,ithChildren.residueType.name];
%                         residueTypeChar = [residueTypeChar,','];
%                         residueTypeChar = [residueTypeChar,ithChildren.ringType.rSize];
%                         
%                         if(~isempty( ithChildren.linkageChildren ))
%                             LinkageChar = ithChildren.linkageParent.bonds.posParent;
%                             LinkageChar = [LinkageChar,...
%                                 ithChildren.anomer.symbol];
%                             LinkageChar = [LinkageChar,...
%                                 ithChildren.anomer.carbonPos];
%                         else
%                             LinkageChar='';
%                         end
%                         
%                         residueLinkageChar = [LinkageChar,residueTypeChar];
%                         residueToRootChar = [residueToRootChar '--' residueLinkageChar];
%                         ischild       = ~isempty(ithChildren.linkageChildren);
%                         if(ischild)
%                             Children = ithChildren.linkageChildren.child;
%                         end
%                     end
%                 end
%             end
%         end
        ...
        function isResidueAdded = addResidue(obj,existresidue,...
                varargin)
            %addResidue add residue to existing residue
            %    addResidue(GLYCANSTRUCTObj,existresidue,residueType,anomerchar,bonds)
            %    addResidue(GLYCANSTRUCTObj,existresidue,residuetoadd,anomerchar,bonds)
            %
            ...
            if(length(varargin)==3)
                ...
                anomerchar = varargin{2};
                bonds      = varargin{3};
                isResidueAdded   = existresidue.addChild(residueType,anomerchar,bonds);
            elseif(length(varargin)==1)
                residuetoadd     = varargin{1};
                isResidueAdded   = existresidue.addChild(residuetoadd);
            elseif(length(varargin)==2)
                residuetoadd     = varargin{1};
                bonds            = varargin{2};
                isResidueAdded   = existresidue.addChild(residuetoadd,bonds);
            end
        ...
        function nredEndResidues= selectTerminalResidue(obj,allResidues)
            ...
            nredEndResidues = allResidues(index);
        end
    ...
    methods
        function new = clone(obj)
            ...
            new = GlycanStruct;
            new.root = obj.root.clone;            
            
            if(~isempty(obj.bracket))
                new.bracket = obj.bracket.clone;
            else
                new.bracket = obj.bracket;
            end
            
            new.name        = obj.name;
            new.attachedMol = obj.attachedMol;
            new.massOptions = obj.massOptions.clone;
            
            if(~isempty(obj.glycanjava))
                new.glycanjava = obj.glycanjava.clone;
            else
                obj.resetjava;
                new.glycanjava = obj.glycanjava.clone;
            end           
        ...
        function isStructEqual = equalStruct(obj,obj2)
            ...
            % See also GlycanStruct
              ...
              if(isempty(obj2.glycanjava))
                obj2.resetjava;  
              end
              
              isStructEqual = obj.glycanjava.equalsStructure(obj2.glycanjava);            
        end
        ...
        function composition=getComposition(obj,varargin)
            ...
            if(isempty(varargin))
                ...
                hasRedEnd  = 0; 
            elseif(length(varargin)==1)
                compOption = varargin{1};
                if(isfield(compOption,'hasRedEnd'))
                    hasRedEnd = varargin{1}.hasredEnd;
                else
                    hasRedEnd = 0;
                end
                
                if(isfield(compOption,'hasBracket'))
                    hasBracket = varargin{1}.hasBracket;
                else
                    hasBracket = 1;
                end
            else
                error('MATLAB:GNAT:ERRORINPUT','ERROR INPUT');
            end
            ...
            if(~hasRedEnd)
                if(isfield(composition,'freeEnd'))
                   composition=rmfield(composition,'freeEnd');
                end  
            end
        end
    end
    ...
    methods
        ...
        isStructEqual = equalGlycanStruct(obj,obj2)        
    end
    
    methods
        glycanJava        = structMat2Java(obj)
        glycanformula     = computeFormula(obj,varargin)
        branchdepth       = getBranchDepth(obj,terminalresidue);
        isBracketAdded    = bracketResidue(obj,residuetobracket) 
        isResidueDeleted  = removeResidueObj(obj,residuetoremove)
        residueToRootChar = getresiduetoroot(obj,residue,varargin)
    end
end
