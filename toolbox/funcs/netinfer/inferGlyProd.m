...
isSubstValid = true;
if(...isprop(enzObj,'glycanTypeSpec')&&
     (~isempty(enzObj.glycanTypeSpec)))  % rule 1: N-, O-, glycolipids
    requirement0 = enzObj.glycanTypeSpec;
    acceptortype  = glycanObj.glycanTypeSpec;
    isSubstValid    = isequal(acceptortype,requirement0);
end

if(...isprop(enzObj,'substNAResidue')&&
    (~isempty(enzObj.substNAResidue)))  %rule 2 It does not contain the specified residue
...
if(isSubstValid)
    if(enzFuncToAdd==1)
        for i = 1 : numRes
            ...
            if(strcmpi(residue.residueType.name,'redEnd'))
                continue;
            end
            
            % rule 3: if the the substrate should target the terminal residue
            if(... isprop(enzObj,'isTerminalTarget')&&
                    ~isempty(enzObj.isTerminalTarget))
                if(enzObj.isTerminalTarget==1)
                    if( ...isprop(residue,'linkageChildren')&&
                        isempty(residue.linkageChildren));
                        isValidRxnPos = true;
                    else
                        isValidRxnPos = false;
                    end
                elseif(enzObj.isTerminalTarget==2)
                     isValidRxnPos = true;
                else
                    if(... isprop(residue,'linkageChildren')&& 
                        isempty(residue.linkageChildren));
            ...
            if(...isprop(enzObj,'substMaxStruct')&&
               (~isempty(enzObj.substMaxStruct)))  % if the rule exists
            ...
            if(...isprop(enzObj,'substMinStruct')&&
               (~isempty(enzObj.substMinStruct)))  % if the rule exists
            ...
            if(...isprop(enzObj,'targetBranch')&&
                (~isempty(enzObj.targetBranch)))  % if the rule exists
            ...
            % specified structure
            if(...isprop(enzObj,'targetbranchcontain')&&
                (~isempty(enzObj.targetbranchcontain)))  % if the rule exists
            ...
            if(... isprop(enzObj,'substNABranch')&&
                ~isempty(enzObj.substNABranch))  % if the rule exists
            ...
            if( ...isprop(enzObj,'targetNABranch')&& 
                 ~isempty(enzObj.targetNABranch))  % if the rule exists
            ...
            if(isResidueAdded)
                ...
                if(... isprop(enzObj,'prodMinStruct')&& 
                    ~isempty(enzObj.prodMinStruct))  % if the rule exists
                ...
                if(... isprop(enzObj,'prodMaxStruct')&&
                      ~isempty(enzObj.prodMaxStruct))  % if the rule exists
        ...
        for i = 1 :numNRERes
            ...
            if(... isprop(enzObj,'substMinStruct')&& 
             ...
             if(~isValidRxnPos)
              ...
              % check requirement 4: the enzyme should not act on the specificed branch
             if(~isempty(enzObj.targetNABranch))  % if the rule exists
                if(isa(enzObj.targetNABranch,'GlycanStruct'))
                    chartargetbranch = enzObj.targetNABranch.name;
                    charresidue      = getresiduetoroot(glycanProdObj,residue);
                    if(length(chartargetbranch)~=length(charresidue))
                        isValidRxnPos = 1;
                    else
                        quotaindex = strfind(charresidue,'?');
                        chartargetbranch(quotaindex) = '?';
                        isValidRxnPos = isValidRxnPos && (~strcmpi(chartargetbranch,charresidue));
                    end
                elseif(isa(enzObj.targetNABranch,'CellArrayList'))
                    for ii = 1 : length(enzObj.targetNABranch)
                        chartargetbranch = enzObj.targetNABranch.name;
            ...
            % check requirement 5: the substrate should not contains the specificed branch
            if(... isprop(enzObj,'substNABranch')&&
                ~isempty(enzObj.substNABranch))  % if the rule exists
                if(isa(enzObj.substNABranch,'GlycanStruct'))
                     isValidRxnPos = isValidRxnPos && (~glycanObj.contains(enzObj.substNABranch));
            ...
            if(isResidueDeleted)
                ...
                if(~isempty(enzObj.prodMinStruct))  % if the rule exists
                    isValidRxnPos = isValidRxnPos &&glycanProdObj.contains(enzObj.prodMinStruct);
                end
